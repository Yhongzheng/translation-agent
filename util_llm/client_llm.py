...
def client_gpt(user_msg: str, sys_msg="", model="gpt-4-turbo", temp=0, res_format="json_object", base_images=None,
               try_count=3):
    """

    :param base_images: 图片信息
    :param user_msg: 主要问题
    :param sys_msg: 系统背景
    :param model: 用的模型
    :param temp: 温度
    :param res_format: 返回的数据格式 有text和json_object
    :param try_count: 重试次数
    :return:
    """
    url = 'http://47.236.142.158:10028/api/gpt'
    # url = 'http://192.168.3.88:10028/api/gpt'
    data = {
        "model": model,
        "sys_msg": sys_msg,
        "user_msg": user_msg,
    ...
    try:
        ...
        if try_count > 0:
            time.sleep(1)
            return client_gpt(user_msg, sys_msg, model, temp, res_format, base_images, try_count - 1)
        else:
            raise e
    if response.status_code != 200:
        err = f"""{"code": {response.status_code}, "msg": {response.text}}"""
        raise Exception(err)
...
def client_gpt_api_key(system_message: str, user_message: str, model: str = "gpt-4o",
                       max_tokens: int = 4096, temperature: int = 0, response_format: str = "text",
    ...
    try:
        ...
        if image_paths:
            ...
            messages[-1]["content"] = [{"type": "text", "text": user_message}, *image_urls]
        ...
        if response.status_code == 200:
            ...
            tokens = json_result['usage']['prompt_tokens']
            return json_result['choices'][0]['message']['content'], tokens
        else:
            return f"Error: {response.status_code}, {response.text}", None
    except Exception as e:
        error_message = f"发送给OpenAI请求时发生错误: {traceback.format_exc()}"
        return error_message, None
