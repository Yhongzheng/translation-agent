...
def num_tokens_in_string(
    ...
    """
    使用指定的编码计算给定字符串中的 tokens 数。

    参数:
        str (str): 要进行 token 化的输入字符串。
...
def split_srt_text(text: str, chunk_size: int) -> List[str]:
    subtitles = text.strip().split('\n\n')
    chunks = []
    current_chunk = ""
    current_chunk_size = 0
    for subtitle in subtitles:
        subtitle_size = num_tokens_in_string(subtitle)
        if current_chunk_size + subtitle_size > chunk_size:
            chunks.append(current_chunk.strip())
            current_chunk = subtitle + '\n\n'
            current_chunk_size = subtitle_size
        else:
            current_chunk += subtitle + '\n\n'
            current_chunk_size += subtitle_size
    if current_chunk:
...
def translate(
    ...
    if num_tokens_in_text < max_tokens:
        ...
        ic(token_size)
        source_text_chunks = split_srt_text(source_text, token_size)  # 使用 split_srt_text 分割文本
